...
pem.createCertificate({days:9999, selfSigned:true}, function(err, keys){
	...
	app.get('/sarah', function(req, res){
    	...
    	if (typeof req.query.reco === 'undefined') {
			...
			res.write("Reco: " + req.query.reco);
			console.log("Reco: " + req.query.reco);
			var url_client_sarah = 'http://127.0.0.1:8080/sarah/SpeechReco' 
			// le client SARAH ne supporte pas les paramètres URIencoded, donc on désencode ...
			var params = {reco:  decodeURI(req.query.reco)};
